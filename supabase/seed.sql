-- ShadowGrid AI — Seed Data
-- Run this AFTER applying the schema migration (00001_create_shadowgrid_schema.sql).
...
-- ─── Citizen Reports ──────────────────────────────────────────────────────────
INSERT INTO citizen_reports (id, report_type, zone_id, severity, description, source, status, created_at)
VALUES
('cr-01', 'street_flooding',    'zone-01', 'critical', 'Water level rising on Elm Street near the underpass — cars unable to pass.', 'citizen', 'pending',      NOW() - INTERVAL '2 hours'),
('cr-02', 'blocked_drainage',   'zone-01', 'high',     'Drain cover on River Ave completely blocked by debris after last night rain.', 'citizen', 'pending',    NOW() - INTERVAL '5 hours'),
('cr-03', 'power_fluctuation',  'zone-02', 'high',     'Lights flickering on 4th and Main. Multiple residents reporting same issue.', 'citizen', 'acknowledged', NOW() - INTERVAL '3 hours'),
('cr-04', 'road_cracks',        'zone-02', 'medium',   'Large crack appearing on Central Ave near the bus terminal.', 'citizen', 'pending',                   NOW() - INTERVAL '8 hours'),
('cr-05', 'street_flooding',    'zone-03', 'high',     'Harbor Rd completely flooded. Access to the port is blocked.', 'citizen', 'acknowledged',              NOW() - INTERVAL '1 hour'),
('cr-06', 'low_water_pressure', 'zone-06', 'medium',   'Water barely trickling from taps in the entire west block since this morning.', 'citizen', 'pending',   NOW() - INTERVAL '4 hours'),
('cr-07', 'traffic_jam',        'zone-05', 'medium',   'Industrial access road completely gridlocked due to truck breakdown.', 'citizen', 'pending',            NOW() - INTERVAL '6 hours'),
('cr-08', 'streetlight_outage', 'zone-04', 'low',      'Three consecutive streetlights out on Northern Blvd since yesterday.', 'citizen', 'pending',            NOW() - INTERVAL '18 hours')
ON CONFLICT (id) DO NOTHING;

-- ─── External Signals ─────────────────────────────────────────────────────────
INSERT INTO external_signals (id, signal_type, zone_id, source, severity, confidence, detected_keywords, title, summary, created_at)
VALUES
...
-- ─── Initial Risk Scores ─────────────────────────────────────────────────────
INSERT INTO risk_scores (id, zone_id, category, score, risk_level, citizen_component, weather_component, signal_component, historical_component, propagation_component, explanation, updated_at)
VALUES
('rs-01', 'zone-01', 'drainage',  88, 'critical', 75, 80, 70, 78, 45, 'Critical drainage risk from heavy rainfall and multiple citizen reports.', NOW()),
('rs-02', 'zone-01', 'emergency_access', 72, 'high', 60, 65, 55, 60, 40, 'Emergency access risk elevated due to road flooding.', NOW()),
('rs-03', 'zone-02', 'power',     65, 'high',     55, 40, 60, 55, 30, 'Power fluctuation signals from citizens and external data.', NOW()),
('rs-04', 'zone-02', 'traffic',   58, 'medium',   40, 30, 45, 50, 25, 'Moderate traffic risk in central business area.', NOW()),
('rs-05', 'zone-03', 'drainage',  82, 'critical', 65, 90, 65, 70, 55, 'Storm alert and coastal flooding driving critical drainage risk.', NOW()),
('rs-06', 'zone-03', 'road',      70, 'high',     45, 60, 55, 65, 35, 'Road damage and harbor closure elevating road risk.', NOW()),
('rs-07', 'zone-04', 'power',     38, 'medium',   25, 20, 30, 28, 15, 'Minor power risk from streetlight outages.', NOW()),
('rs-08', 'zone-05', 'traffic',   62, 'high',     45, 20, 55, 65, 20, 'Traffic disruption from industrial vehicle incident.', NOW()),
('rs-09', 'zone-05', 'road',      55, 'medium',   35, 15, 40, 60, 18, 'Road wear from heavy machinery traffic.', NOW()),
('rs-10', 'zone-06', 'water',     60, 'high',     50, 25, 55, 42, 20, 'Water pressure shortage affecting residential area.', NOW()),
('rs-11', 'zone-06', 'drainage',  48, 'medium',   35, 45, 35, 40, 30, 'Moderate drainage risk from elevated flood sensitivity.', NOW()),
('rs-12', 'zone-07', 'drainage',  22, 'low',      10, 15, 10, 22, 12, 'Low drainage risk due to elevated terrain.', NOW()),
('rs-13', 'zone-07', 'traffic',   28, 'low',      20, 10, 15, 22, 10, 'Minor traffic congestion near university campus.', NOW())
ON CONFLICT (id) DO NOTHING;

-- ─── Recommendations ──────────────────────────────────────────────────────────
INSERT INTO recommendations (id, zone_id, category, action_type, title, description, priority, urgency, expected_impact_reduction, status)
VALUES
('rec-01', 'zone-01', 'drainage',         'emergency_pump_deployment', 'Deploy Emergency Drainage Pumps — Riverside District',
 'Activate mobile drainage pumps at River Ave and Elm Street. Current drain blockage and rainfall create critical overflow risk.', 1, 'immediate', 35, 'pending'),
('rec-02', 'zone-03', 'drainage',         'emergency_pump_deployment', 'Emergency Pump Deployment — Old Harbor Quarter',
 'Storm alert and coastal flooding require immediate pump activation near harbor entrance.', 2, 'immediate', 40, 'pending'),
('rec-03', 'zone-02', 'power',            'inspect_power_lines',       'Inspect Substation Overload — Central Business Hub',
 'Multiple reports of power fluctuation. Grid operator should inspect primary substation for overload conditions.', 3, 'urgent', 30, 'pending'),
('rec-04', 'zone-01', 'emergency_access', 'protect_hospital_access',   'Clear Emergency Routes — Riverside District',
 'Verify all emergency vehicle routes remain accessible. Coordinate with flood response teams to maintain hospital access.', 4, 'urgent', 25, 'pending'),
('rec-05', 'zone-06', 'water',            'water_supply_check',        'Pressure Investigation — Westbank Residential',
 'Dispatch water utility team to identify source of pressure drop. Check distribution node at West Main junction.', 5, 'urgent', 20, 'pending'),
('rec-06', 'zone-05', 'traffic',          'redirect_traffic',          'Traffic Diversion — Industrial East Zone',
 'Activate alternate route signage for industrial access road. Coordinate with traffic management centre.', 6, 'scheduled', 15, 'pending'),
('rec-07', 'zone-03', 'road',             'road_inspection',           'Road Structural Inspection — Harbor Access',
 'Full structural assessment of Harbor Rd required before normal traffic is restored.', 7, 'scheduled', 20, 'pending')
ON CONFLICT (id) DO NOTHING;

-- ─── Failure Chains ───────────────────────────────────────────────────────────
INSERT INTO failure_chains (id, title, trigger, trigger_zone_id, steps, overall_risk_level, is_active)
VALUES
...
-- ─── Team Allocations ─────────────────────────────────────────────────────────
INSERT INTO team_allocations (id, zone_id, team_type, team_count, priority_rank, expected_risk_reduction, deployment_notes, status)
VALUES
('ta-01', 'zone-01', 'Drainage Response Team',   3, 1, 32, 'Deploy to River Ave and Elm Street outfall points. Bring 2 mobile pumps.', 'planned'),
('ta-02', 'zone-03', 'Coastal Emergency Unit',   2, 2, 38, 'Focus on harbor entrance seawall. Coordinate with port authority.', 'planned'),
('ta-03', 'zone-02', 'Power Grid Inspection',    1, 3, 28, 'Inspect Central substation and 4th/Main junction box.', 'planned'),
('ta-04', 'zone-06', 'Water Utility Team',       1, 4, 22, 'Locate pressure fault at West Main distribution node.', 'planned'),
('ta-05', 'zone-05', 'Traffic Management Unit',  1, 5, 14, 'Set up diversion signage on industrial access road.', 'planned')
ON CONFLICT (id) DO NOTHING;
